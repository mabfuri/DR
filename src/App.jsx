...
import { getAdsterraStats } from './lib/adsterraStats'
...
function Dashboard({ profile }) {
  ...
  const [offers,setOffers]=useState([]); const [index,setIndex]=useState(0); const [offerError,setOfferError]=useState(''); const [balance,setBalance]=useState(0); const [accumulated,setAccumulated]=useState(0); const [rewardNotice,setRewardNotice]=useState(''); const [rewarding,setRewarding]=useState(false); const [limitedOffers,setLimitedOffers]=useState({}); const [withdrawOpen,setWithdrawOpen]=useState(false); const [withdrawAmount,setWithdrawAmount]=useState(''); const [bankName,setBankName]=useState(''); const [accountNumber,setAccountNumber]=useState(''); const [accountName,setAccountName]=useState(''); const [withdrawNotice,setWithdrawNotice]=useState(''); const [withdrawError,setWithdrawError]=useState(''); const [withdrawing,setWithdrawing]=useState(false);
  const [stats,setStats]=useState([['Impressions','0'],['Clicks','0'],['CTR','0'],['CPM','0'],['Revenue','Rp0']]); const [statsError,setStatsError]=useState('');
  const unlockInProgressRef=useRef(false);
  const limitedOffersRef=useRef(limitedOffers);
