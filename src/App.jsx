  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (offersErr) setOfferError(offersErr.message)
        ...
        let offerRows = []
        if (ids.length) {
          const { data, error } = await supabase.from('offers').select('id,title,link,user_id,sort_order').in('id',ids).eq('status','active').order('sort_order',{ascending:true}).order('created_at',{ascending:false})
