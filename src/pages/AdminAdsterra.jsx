...
export default function AdminAdsterra() {
  ...
  async function savePlacement(user) {
    ...
    if (updateError) setError(updateError.message)
      ...
      setNotice(`Placement ID ${user.username || 'user'} berhasil disimpan.`)
    }
    setSaving(null)
  }
  ...
  async function loadStats(user) {
    const placement = String(user.adsterra_placement_id || '').trim()
    if (!placement) { setError(`Placement ID ${user.username || 'user'} belum diisi.`); return }
    setLoadingStats(user.id); setError(''); setNotice('')
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const token = sessionData?.session?.access_token
      if (!token) throw new Error('Sesi admin tidak ditemukan.')
      const response = await fetch(`/api/adsterra-stats?placement=${encodeURIComponent(placement)}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      const body = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(body?.error || 'Gagal mengambil statistik Adsterra.')
  ...
  return <section className="admin-users">
    <div className="card">
      ...
      <p className="muted">API Key Adsterra dipanggil hanya dari Cloudflare Function. API Key tidak dikirim ke browser. Periode default menggunakan seluruh rentang yang didukung endpoint.</p>
    </div>
    ...
    <div style={{display:'grid',gap:10,marginTop:12}}>
      {filtered.map(user => {
        ...
        return <article className="card" key={user.id} style={{display:'grid',gap:12}}>
          <div style={{display:'grid',gridTemplateColumns:'minmax(150px,1fr) minmax(180px,280px) auto',gap:12,alignItems:'end'}}>
            ...
            <div style={{display:'flex',gap:8}}><button type="button" disabled={saving === user.id} onClick={() => savePlacement(user)}>{saving === user.id ? 'MENYIMPAN...' : 'Simpan'}</button><button type="button" className="ghost" disabled={loadingStats === user.id || !user.adsterra_placement_id} onClick={() => loadStats(user)}>{loadingStats === user.id ? 'MEMUAT...' : 'Statistik'}</button></div>
          </div>
